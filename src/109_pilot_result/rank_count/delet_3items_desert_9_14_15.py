...
# 读取CSV文件
file_path = r'C:\Users\zqy\Documents\over regression\over_behavior_trace\src\109_pilot_result\rank_count\rank_desert.csv'
...
# 定义A组和D组
group_alone = ['A', 'D']
correct_ranks = [4,	6,	12,	7,	11,	10,	8,	5,	15,	3,	13,	9,	14]
...
print(f"task with AI desert(B&C): {score_dict_remove3_withAI}")

score_dict_remove3_alone = {}
for participant, scores in remove3_scores_alone:
    total_difference = calculate_total_difference(correct_ranks, scores)
    score_dict_remove3_alone[participant.strip('(')] = total_difference
print(f'task alone desert(A&D): {score_dict_remove3_alone}')
...
print(f"Average score with AI desert(B&C): {average_score_list_remove3_withAI}")
print(f"Average score alone desert(A&D): {average_score_list_remove3_alone}")
